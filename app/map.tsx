...
export default function MapScreen() {
  ...
  const checkGeofenceStatus = useCallback(async (childLoc: ChildLocation | null, activeGeofences: Geofence[]) => {
    if (!childLoc || activeGeofences.length === 0) {
      ...
      setPreviousGeofenceStatus('unknown');
      return;
    }
    ...
    // Notification Logic for Geofence
    if (previousGeofenceStatus !== 'unknown') {
  ...
  const fetchLocationAndGeofenceData = async () => {
    ...
    try {
      ...
      const ldrRef = ref(db, `child_status/${deviceId}/ldr_removed`);

      const [locationSnapshot, geofencesSnapshot, ldrSnapshot] = await Promise.all([
        get(locationRef),
        get(geofencesRef),
        get(ldrRef)
      ]);
      ...
      // Process LDR Status
      if (ldrSnapshot.exists()) {
        const data = ldrSnapshot.val();
        setLdrStatus(data);
        console.log('LDR status data received:', data);
      } else {
        setLdrStatus(null);
        console.log('No LDR status data found');
      }
  ...
  useEffect(() => {
    ...
    const requestPermissions = async () => {
      ...
      if (finalStatus !== 'granted') {
        Alert.alert('Permission required', 'Push notifications are required to alert you about safe zone changes and device removal.');
        return;
      }
    };
  ...
  useEffect(() => {
    ...
    const ldrRef = ref(db, `child_status/${deviceId}/ldr_removed`);
    const unsubscribeLdr = onValue(ldrRef, async (snapshot) => {
      const data = snapshot.val();
      if (data) {
        setLdrStatus(data);
        // Trigger notification if status changes to removed
        if (data.removed && previousLdrRemoved === false) {
          await Notifications.scheduleNotificationAsync({
            content: {
              title: "Device Alert!",
              body: "Your child's device has been removed!",
              data: { type: 'device_removed' },
            },
            trigger: null,
          });
          console.log("Notification: Child's device removed");
        } else if (!data.removed && previousLdrRemoved === true) {
          await Notifications.scheduleNotificationAsync({
            content: {
              title: "Device Alert!",
              body: "Your child's device has been put back on.",
              data: { type: 'device_reattached' },
            },
            trigger: null,
          });
          console.log("Notification: Child's device reattached");
        }
        setPreviousLdrRemoved(data.removed); // Update previous state
      } else {
        setLdrStatus(null);
    ...
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>
          {pairingLoading ? "Checking pairing status..." : "Finding your child's location..."}
        </Text>
        ...
        <Pressable
          ...
          onPress={() => {
            ...
            setLdrStatus({ removed: false, timestamp: new Date().toISOString() }); // Set dummy LDR status
          }}
        >
    ...
    return (
      <View style={styles.noDataContainer}>
        ...
        <Text style={styles.noDataSubtext}>
          To view your child's real-time location, please pair with their device.
        </Text>
    ...
    <TouchableWithoutFeedback onPress={() => {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.infoContainer}>
          {/* Device Removal Status (LDR) */}
          <View style={styles.statusSection}>
            <Ionicons
              name={ldrStatus?.removed ? 'alert-circle' : 'checkmark-circle'}
              size={20}
              color={ldrStatus?.removed ? '#FF3B30' : '#4CAF50'}
            />
            <Text style={[
              styles.statusText,
              { color: ldrStatus?.removed ? '#FF3B30' : '#4CAF50' }
            ]}>
              Device Status: {ldrStatus?.removed ? 'Removed from arm!' : 'On arm'}
            </Text>
            {/* REMOVED: {ldrStatus?.timestamp && (
              <Text style={styles.statusTimestamp}>
                ({new Date(ldrStatus.timestamp).toLocaleTimeString()})
              </Text>
            )} */}
          </View>
...
const styles = StyleSheet.create({
  ...
  statusTimestamp: {
    fontSize: 12,
    color: '#666',
    marginLeft: 'auto', // Pushes timestamp to the right
  },
  updateInfo: {
    fontSize: 12,
    color: '#666'
