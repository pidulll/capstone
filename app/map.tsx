// app/map.tsx - With search pin functionality and Geofence Notifications
import React, { useState, useEffect, useRef } from 'react';
import {
  View, Text, StyleSheet, Alert, ActivityIndicator, Dimensions,
  Pressable, Keyboard, TouchableWithoutFeedback, Platform
} from 'react-native';
...
import { ref, get, onValue, off } from 'firebase/database'; // Added onValue, off
...
export default function MapScreen() {
  ...
  const isGeofenceBreachedRef = useRef(false); // Use ref to track breach status across renders
  ...
  const sendGeofenceNotification = async () => {
    await Notifications.scheduleNotificationAsync({
      content: {
        ...
        body: "Your child has exited a designated safe zone.",
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (finalStatus !== 'granted') {
        Alert.alert('Notification Permission', 'Please enable notification permissions to receive geofence alerts.');
      }
    })();

    fetchLocationData();
    const interval = setInterval(fetchLocationData, 60000); // Refresh location every minute

    // --- Firebase Listener for Geofence Breach Status ---
    const geofenceStatusRef = ref(db, 'geofence_status/breached');
    const unsubscribe = onValue(geofenceStatusRef, (snapshot) => {
      const isBreached = snapshot.val();
      if (isBreached === true && isGeofenceBreachedRef.current === false) {
        // Only send notification if status changes from not breached to breached
        sendGeofenceNotification();
      }
      isGeofenceBreachedRef.current = isBreached; // Update ref with current status
      console.log("Geofence breached status from Firebase:", isBreached);
    });

    return () => {
      clearInterval(interval);
      off(geofenceStatusRef); // Clean up Firebase listener
    };
  }, []);
      ...
      <View style={styles.container}>
        ...
        <View style={styles.infoContainer}>
          ...
          <View style={styles.locationDetails}>
            ...
            {isGeofenceBreachedRef.current && (
              <View style={styles.breachWarning}>
                <Ionicons name="warning" size={20} color="#FF3B30" />
                <Text style={styles.breachWarningText}>Child is outside a safe zone!</Text>
              </View>
            )}
          </View>
        </View>
